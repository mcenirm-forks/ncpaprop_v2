...
#include "cmodess_parameters.h"
#include <string>
#include <iostream>

void NCPA::configure_cmodess_parameter_set( NCPA::ParameterSet *ps ) {
	...
	ps->addHeaderTextVerbatim("|                          Complex Normal Modes                            |");
	...
	ps->addHeaderText("file tloss_1d.cnm - considering attenuation in the atmosphere" );
	ps->addHeaderText("file tloss_1d.lossless.cnm  - no attenuation" );
	ps->resetHeaderIndent();
	ps->addHeaderText("Additionally, if the flag --write_2d_tloss is present on the command line, the 2D TL is saved to file tloss_2d.cnm. The user can also choose to propagate in N different directions i.e. (N by 2D mode) by using the option --multiprop.");
	ps->addBlankHeaderLine();
	ps->addHeaderText("The options below can be specified in a colon-separated file \"cmodess.param\" or at the command line. Command-line options override file options.");
	...
	ps->addParameter( new StringParameter( "paramfile", "cmodess.param") );
...
*/
	...
	*/
	...
	ps->addFooterTextVerbatim("  tloss_1d.cnm:                r, 4*PI*Re(P), 4*PI*Im(P), (incoherent TL)");
	ps->addFooterTextVerbatim("  tloss_1d.lossless.cnm:");
	ps->addFooterTextVerbatim("  tloss_2d.cnm:                r, z, 4*PI*Re(P), 4*PI*Im(P)");
	ps->addFooterTextVerbatim("  Nby2D_tloss_1d.cnm:");
	ps->addFooterTextVerbatim("  Nby2D_tloss_1d.lossless.cnm: r, theta, 4*PI*Re(P), 4*PI*Im(P), (incoherent TL)");
	ps->addFooterTextVerbatim("  phasespeeds.cnm:             Mode#, phase speed [m/s], imag(k)");
	ps->addFooterTextVerbatim("  speeds.cnm:                  Mode#, phase speed [m/s], group speed [m/s], imag(k)");
	ps->addFooterTextVerbatim("  mode_<mode_count>.cnm        z, (Mode amplitude)");
	ps->addFooterTextVerbatim("  dispersion_<freq>.cnm        Contains one line with entries:");
	...
	ps->addFooterTextVerbatim("  atm_profile.cnm              z,u,v,w,t,d,p,c,c_eff");
	ps->addBlankFooterLine();
	...
	ps->addFooterText("../bin/CModess --singleprop --atmosfile NCPA_canonical_profile_zuvwtdp.dat --azimuth 90 --freq 0.1" );
	ps->addBlankFooterLine();
	ps->addFooterText("../bin/CModess --singleprop --atmosfile NCPA_canonical_profile_zuvwtdp.dat --azimuth 90 --freq 0.1 --write_2d_tloss" );
	ps->addBlankFooterLine();
	ps->addFooterText("../bin/CModess --multiprop --atmosfile NCPA_canonical_profile_zuvwtdp.dat --freq 0.1 --azimuth_start 0 --azimuth_end 360 --azimuth_step 1" );
