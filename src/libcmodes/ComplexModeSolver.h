#ifndef NCPAPROP_COMPLEXMODESOLVER_H_INCLUDED
#define NCPAPROP_COMPLEXMODESOLVER_H_INCLUDED
...
namespace NCPA {
	class ComplexModeSolver : public AtmosphericTransferFunctionSolver {

	public:           

        // constructor/destructor
		virtual ~ComplexModeSolver();
		...
		// general functions
		int getNumberOfModes(int n, double dz, std::complex< double > *diag, double k_min, double k_max, int *nev);
		int sturmCount(int n, double dz, std::complex< double > *diag, double k, int *cnt);	
		int getTLoss1D(int select_modes, double dz, int n_r, double dr, double z_src, double z_rcv, 
			double *rho, std::complex<double> *k_s, std::complex<double> **v_s, std::string filename_lossy, 
			std::string filename_lossless);
		int getTLoss1DNx2(double azimuth, int select_modes, double dz, int n_r, double dr, double z_src, 
			double z_rcv,  double *rho, std::complex<double> *k_s, std::complex<double> **v_s, bool Nx2, int iter,
		...
		int getTLoss2D(int nz, int select_modes, double dz, int n_r, double dr, double z_src, 
			double *rho, std::complex<double> *k_pert, std::complex<double> **v_s, std::string filename_lossy ); 
		int writeDispersion(int select_modes, double dz, double z_src, double z_rcv, double freq, 
			std::complex<double> *k_pert, std::complex<double> **v_s, double *rho);
		int writeDispersion(FILE *fp, int select_modes, double dz, double z_src, double z_rcv, 
			double freq, std::complex<double> *k_pert, std::complex<double> **v_s, double *rho);
		int writePhaseSpeeds(int select_modes, double freq, std::complex<double> *k_pert);
		int writeEigenFunctions(int nz, int select_modes, double dz, std::complex<double> **v_s);
		int doNormalize( int nz, int n_modes, double dz, std::complex<double> **v_s );

		virtual int doSelect( int nz, int n_modes, double k_min, double k_max, std::complex< double > *k2, 
