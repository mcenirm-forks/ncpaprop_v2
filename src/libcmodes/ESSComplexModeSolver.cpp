...
#include "util.h"
#include "ESSComplexModeSolver.h"
...
NCPA::ESSComplexModeSolver::ESSComplexModeSolver( NCPA::ParameterSet *param, NCPA::Atmosphere1D *atm_profile )
{
	setParams( param, atm_profile );           
}

NCPA::ESSComplexModeSolver::~ESSComplexModeSolver() {}

void NCPA::ESSComplexModeSolver::setParams( NCPA::ParameterSet *param, NCPA::Atmosphere1D *atm_prof )
{		
	...
	if (write_phase_speeds || write_2D_TLoss || write_modes || (dispersion_file.size() > 0)) {
...
int NCPA::ESSComplexModeSolver::solve() {
	//
	...
	*/
	...
	double k_min, k_max;			
	std::complex<double> *diag, *k2, *k_s, **v, **v_s;

	diag   = new std::complex<double> [Nz_grid];
  
	k2     = new std::complex<double> [MAX_MODES];
	k_s    = new std::complex<double> [MAX_MODES];
	v      = NCPA::cmatrix(Nz_grid,MAX_MODES);
	v_s    = NCPA::cmatrix(Nz_grid,MAX_MODES); 			
	...
	for (fi = 0; fi < Nfreq; fi++) {
		...
		for (it=0; it<Naz; it++) {
			...
			atm_profile->get_property_vector( "_ALPHA_", alpha );
			//
	    	...
	    	// std::ofstream fout("diag_new.dat");
	    	// for (int iii = 0; iii < Nz_grid; iii++) {
	    	// 	fout << iii << " " << diag[ iii ] << std::endl;
	    	// }
	    	// fout.close();

	    	i = NCPA::ComplexEigenEngine::doESSCalculation( diag, Nz_grid, dz, tol, &nev,
			...
			if (Nby2Dprop) { // if (N by 2D is requested)
				getTLoss1DNx2(azi, select_modes, dz, Nrng_steps, rng_step, sourceheight, 
					      receiverheight, rho, k_s, v_s, Nby2Dprop, it,
					      "Nby2D_tloss_1d.cnm", "Nby2D_tloss_1d.lossless.cnm" );
			}
			else {
				...
				getTLoss1D(select_modes, dz, Nrng_steps, rng_step, sourceheight, receiverheight, rho, 
					   k_s, v_s, "tloss_1d.cnm", "tloss_1d.lossless.cnm" );
	...
	delete[] k_s;
	// delete[] k_pert;
	NCPA::free_cmatrix(v, Nz_grid, MAX_MODES);
	NCPA::free_cmatrix(v_s, Nz_grid, MAX_MODES);
...
int NCPA::ESSComplexModeSolver::getModalTrace( int nz, double z_min, double sourceheight, double receiverheight, 
	double dz, Atmosphere1D *p, double admittance, double freq, double azi, std::complex<double> *diag,
	...
	std::complex<double> k_eff;
	std::complex<double> I( 0.0, 1.0 );
	double tweak_abs = 1.0;
	z_min_km = z_min / 1000.0;
	dz_km    = dz / 1000.0;
	omega    = 2*PI*freq;
	double alpha;
	...
	for (i=0; i<nz; i++) {	     
		...
		alpha = p->get( "_ALPHA_", Hgt[ i ] );
		k_eff = omega / ceffz[ i ] + I * tweak_abs * alpha;
		diag[ i ] = k_eff * k_eff;

		// std::cout << i << " " << Hgt[i] << " " << alpha << " " << diag[ i ] << std::endl;
...
int NCPA::ESSComplexModeSolver::doSelect(int nz, int n_modes, double k_min, double k_max, 
	std::complex<double> *k2, std::complex<double> **v, std::complex<double> *k_s, 
	std::complex<double> **v_s, int *select_modes)
{
	int cnt = -1;
	int i, j;
	double k;
	for (j=0; j<n_modes; j++) {
		k = std::sqrt(k2[j]).real();
		if ((k >= k_min) && (k <= k_max)) {
			cnt++;
			for (i=0; i<nz; i++) {
				v_s[i][cnt] = v[i][j];
			}
			k_s[cnt] = std::sqrt( k2[ j ] );
		}
	}
...
// Modification to apply the sqrt(k0) factor to agree with Jelle's getModalStarter; 
// this in turn will make 'pape' agree with modess output
// void NCPA::ESSComplexModeSolver::getModalStarter(int nz, int select_modes, double dz, double freq,  
...
// int NCPA::ESSComplexModeSolver::writePhaseAndGroupSpeeds(int nz, double dz, int select_modes, double freq, 
